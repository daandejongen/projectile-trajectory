...
type staticAngleThrustForceWithLimitedDurationCalculator struct {
	force    linalg.Vector2d
	duration float64
}

func newThrustForceCalculator(thrust Thrust) thrustForceCalculator {
	return staticAngleThrustForceWithLimitedDurationCalculator{
		force:    linalg.NewVectorFromLengthAndAngle(thrust.Force, thrust.Angle),
		duration: thrust.Duration,
	}
}

func (calculator staticAngleThrustForceWithLimitedDurationCalculator) compute(velocity linalg.Vector2d, timePassed float64) linalg.Vector2d {
